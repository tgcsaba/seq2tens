...
def run(_run, dataset, model_name, model_hparams, train_hparams, follow_test_metrics=False, tmp_dir='./tmp/'):
    ...
    logger_cb = seq2tens.utils.SacredLoggerCallback(_run)
    validation_data = (X_test, y_test) if follow_test_metrics else None
    history = model.fit(X_train, y_train, balance_loss=True, callbacks=[logger_cb], validation_data=validation_data)
    ...
    train_loss, train_acc =  model.evaluate(X_train, y_train, batch_size=model.get_default_batch_size(dataset['n_train']), balance_loss=True)
    _run.log_scalar('training.final_loss', train_loss)
    _run.log_scalar('training.final_accuracy', train_acc)
    
    test_loss, test_acc = model.evaluate(X_test, y_test, batch_size=model.get_batch_size(dataset['n_test']))
